...
void testBufferedStreamReaderDirect() {
  ...
  // String-based BufferedStreamReader tests
  {
    BufferedStreamReader strReader("abc");
    expectTrue(strReader.available(), "strReader available");
    expectEqInt(strReader.peek(), 'a', "strReader peek 'a'");
    expectEqInt(strReader.read(), 'a', "strReader read 'a'");
    expectEqInt(strReader.read(), 'b', "strReader read 'b'");
    expectEqInt(strReader.read(), 'c', "strReader read 'c'");
    expectTrue(!strReader.available(), "strReader no longer available");
    expectEqInt(strReader.read(), -1, "strReader read past end returns -1");
    expectEqInt(strReader.peek(), -1, "strReader peek past end returns -1");

    BufferedStreamReader emptyReader("");
    expectTrue(!emptyReader.available(), "emptyReader not available");
    expectEqInt(emptyReader.read(), -1, "emptyReader read returns -1");
    expectEqInt(emptyReader.peek(), -1, "emptyReader peek returns -1");
  }

  // Stream reader read/peek EOF test
  {
    MockDirectStream emptyStream("");
    BufferedStreamReader streamReader(&emptyStream, false);
    expectTrue(!streamReader.available(), "streamReader empty available is false");
    expectEqInt(streamReader.read(), -1, "streamReader empty read is -1");
    expectEqInt(streamReader.peek(), -1, "streamReader empty peek is -1");
  }

  // Test readRawByte timeout directly via chunked reader with timeout stream
  {
    MockDirectStream timeoutStream2;
    timeoutStream2.availableTimesOut = true;
    BufferedStreamReader chunkReader(&timeoutStream2, true);
    expectTrue(!chunkReader.available(), "chunkReader timeout available is false");
  }
